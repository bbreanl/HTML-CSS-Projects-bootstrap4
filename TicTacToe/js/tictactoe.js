...
let activePlayer = 'X';
//This array stores an array of moves. We use this to determaine win conditions. 
let selectedSquares = [];
...
function placexoro(squareNumber) {
    ...
    if(!selectedSquares.some(element => element.includes(squareNumber))){
        ...
        //This function playes placement sound. audio('./media/place.mp3');
    ...
    function computersTurn(){
        ...
        let success = false;
        //This variable stores a radom number 0-8.
        let pickASquare;
        //This  condition allows our while loop to keep trying if a square is selected already. 
        while (!success) {
            //A Sber between 0 and Sted. 
...
function drawWinLine(coordX1, coordY1,coordX2, coordY2) {
  ...
  //This line indicates where the end of a lines x axis is.
  y2 = coordY2,
  //This variable stores temporary x axis data we updated in our animation loop.
  x = x1,
...
function animatelineDrawing() {
    ...
    if (x1 <= x2 && y1 <= y2) {
        ...
        //This is necessary for the 6, 4, 2 win condidtions.
        if (x >= x2 && y >= y2) { cancelAnimationFrame(animationLoop); }
    }
    //This condition is similar to the one above.
    //THis is necessary for thr 6, 4, 2 win condition.
    if (x1 <= x2 && y1 >= y2) {
        if (x < x2) { x += 10; }
        if (y > y2) { y -= 10; }
        if (x >= x2 && y <= y2) { cancelAnimationFrame(animationLoop)}
